...
def main():
    data = pd.read_csv('exerc.data', names=['class', 'atr1', 'atr2', 'atr3', 'atr4',
                       'atr5', 'atr6', 'atr7', 'atr8', 'atr9', 'atr10', 'atr11', 'atr12', 'atr13'])

    x_train, x_test = train_test_split(
        data, test_size=0.2, random_state=32, shuffle=True)
    ...
    for i in range(np.shape(x_test)[0]):
        ...
        for j in range(k):
            most_classes_close[j], y_verification = most_close_element(
                x_test[i], x_train, y_train, y_verification)
